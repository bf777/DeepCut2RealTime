...
@click.option('-c','--save','save_as_csv',
              is_flag=True,
              help='Saves as a .csv file. Default is False.')
@click.option('-c','--save','save_frames',
              is_flag=True,
              help='Saves labelled frames of stream. Default is False.')
@click.option('-b','--baseline','baseline',
              is_flag=True,
              help='Set to False to provide feedback to subject on GPIO port 7 (e.g. water reward).')
@click.option('-n','--name','name',
            is_flag=True,
            help='Takes the ID of the current subject as a string.')
@click.pass_context
def analyze_stream(_, *args,**kwargs):

    """Makes prediction on video stream.\n
        CONFIG: Full path of the "config.yaml" file in the train directory of a project.\n
        DESTFOLDER: Folder for saving data and (optionally) labelled frames.\n

    Example\n
    ----------

    python3 dlc.py analyze_stream /home/project/reaching/config.yaml /home/project/output

    """
    from deeplabcut.pose_estimation_tensorflow import predict_stream
    predict_stream.analyze_stream(*args,**kwargs)
###########################################################################################################################
